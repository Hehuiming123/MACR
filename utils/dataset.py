...
class CUB():
    def __init__(self, root, is_train=True, data_len=None, transform=None):
        ...
        img_txt_file = open(os.path.join(self.root, 'images.txt').replace('\\','/'))
        label_txt_file = open(os.path.join(self.root, 'image_class_labels.txt').replace('\\','/'))
        train_val_file = open(os.path.join(self.root, 'train_test_split.txt').replace('\\','/'))
        img_name_list = []
