...
class Encoder(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        weight = torch.eye(785).repeat(batch_size, 1, 1).to(device)
        eye = torch.eye(785).unsqueeze(0).to(device)
        out10, out11 = None, None

        for i in range(len(self.layer)):
            hidden_states, w = self.layer[i](hidden_states)
            if i == 9:
                out10 = hidden_states
            elif i == 10:
                out11 = hidden_states
        ...
        num = hidden_states.size(1) - 1
        attn = weight[:, 0, 1:]
        _, idx = attn.sort(dim=-1, descending=True)
        idx = idx[:, :int(num * self.eta)] + 1
        cls_token = out11[:, 0].unsqueeze(1)
        content_token = []
        for i in range(batch_size):
            content_token.append(out11[i][idx[i, :], :])
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, config, img_size=224, num_classes=200):
        ...
        self.min_crop_ratio = 0.2
        self.patch_size = config['patches']['size'][0]


    def get_attention_map(self, attn_weights):
        ...
        response_map = F.interpolate(
            attn_weights.view(B, 1, H, W),
            scale_factor=self.patch_size,
            mode='bilinear',
            align_corners=True
        )
        return response_map.squeeze(1)

    def crop_object(self, x, attn_weights):
        B, C, H, W = x.shape
        response_map = self.get_attention_map(attn_weights[:, 0, 1:])  # [B, H, W]
        ...
        for i in range(B):
            threshold = (response_map[i].mean() + 0.3 * response_map[i].max()) * 0.7

            binary_map = (response_map[i] > threshold).float().cpu().numpy()
            ...
            if not regions:
                min_size = int(min(H, W) * self.min_crop_ratio)
                size = max(min_size, int(min(H, W)*0.2))
                y_min = max(0, (H - size) // 2)
                x_min = max(0, (W - size) // 2)
                y_max = min(H, y_min + size)
                x_max = min(W, x_min + size)
            else:
                largest = max(regions, key=lambda r: r.area)
                y_min, x_min, y_max, x_max = largest.bbox
                ...
                if h < min_h:
                    pad = max(int((min_h - h) * 0.3), 1)
                    y_min = max(0, y_min - pad)
                    y_max = min(H, y_max + pad)

                if w < min_w:
                    pad = max(int((min_w - w) * 0.3), 1)
                    x_min = max(0, x_min - pad)
                    x_max = min(W, x_max + pad)

            crop = x[i:i+1, :, y_min:y_max, x_min:x_max]
            crop = F.interpolate(crop, size=(H, W), mode='bilinear', align_corners=True)

            fore_x[i] = crop
            back_x[i:i+1, :, y_min:y_max, x_min:x_max] = 0
            crop_coords.append((x_min, y_min, x_max, y_max))

        return fore_x, back_x, crop_coords
    ...
    def forward(self, x, swap=False):
        if swap:
            ...
            fore_x, back_x, crop_coords = self.crop_object(x, weight)
            batch_size = x.size()[0]
            image = x
            p = self.img_size // self.patch_size
            image_new = image.detach().clone()
            weight = weight[:, 0, 1:]
            weight = weight / torch.amax(weight, dim=-1, keepdim=True)
            weight = weight.view(batch_size, 1, p, p)

            p = self.p
            ratio = self.img_size // p
            weight = F.adaptive_avg_pool2d(weight, (p, p)).view(batch_size, -1)
            weight = weight / weight.sum(dim=-1, keepdim=True)
            score, indices = torch.sort(weight, descending=True, dim=-1)
            ...
            return out10, out11, out12, out13, fore_x, back_x, crop_coords, image_new, lam_a, lam_b, rand_index
        else:
            out10, out11, out12, out13, _ = self.transformer(x)
            out10 = self.part_head10(out10[:, 0])
            out11 = self.part_head11(out11[:, 0])
            out12 = self.part_head12(out12[:, 0])
            out13 = self.part_head13(out13[:, 0])
...
if __name__ == "__main__":
    ...
    model = model.cuda()
    test_img = torch.randn(1, 3, 448, 448).cuda()
    with torch.no_grad():
        out110, out111, out112, out113, fore_x, back_x, crop_coords, image_new, lam_a, lam_b, rand_index = model(test_img, swap=True)
